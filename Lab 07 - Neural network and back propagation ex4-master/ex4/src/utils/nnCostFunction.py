...
# Setup some useful variables
    ...
    i2 = X.dot(Theta1.T)
    o2 = np.c_[np.ones((m, 1)), sigmoid(i2)]
    i3 = o2.dot(Theta2.T)
    o3 = sigmoid(i3)
        
    J=0
    for i in range(0, m-1):
        one_hot = np.zeros((num_labels, 1))
        one_hot[y[i]-1] = 1
        J += (-np.log(o3[i]).dot(one_hot) - (np.log(1-o3[i]).dot((1-one_hot))))

    reg = (Lambda/(2*m))*(sum(sum(np.square(Theta1[:,1:])))+ sum(sum(np.square(Theta2[:,1:]))))
    J = (J /m) + reg

    for i in range(0, m-1):
        one_hot = np.zeros((1,num_labels))
        one_hot[0,y[i]-1] = 1
        
        d3 = o3[i,:] - one_hot
        d2 = (Theta2.T.dot(d3.T).T * sigmoidGradient([1, i2[i,:]]).T ).T
        
        Theta1_grad = Theta1_grad + d2[1:] *X[i,:]
        Theta2_grad = Theta2_grad + d3.T * o2[i,:]
